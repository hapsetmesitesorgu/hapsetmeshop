  ...
  card.innerHTML = `
    ...
    <div class="card-header">
      ...
      <div class="card-header-actions">
        <button class="btn-fav" id="btn-fav-${api.id}" onclick="toggleFavorite('${api.id}')" title="Favorilere ekle">
      ...
      <div class="result-header">
        ...
        <div class="result-actions">
          ...
          <div class="export-btns">
            <button class="btn-export" onclick="exportResult('${api.id}', 'json')" title="JSON indir">
              <i class="fa-solid fa-file-code"></i> JSON
            </button>
            <button class="btn-export" onclick="exportResult('${api.id}', 'txt')" title="TXT indir">
...
/* ═══════════════════════════════════════════
   DIŞA AKTARMA (JSON / TXT)
═══════════════════════════════════════════ */

/**
 * Sonuç kutusundaki veriyi dosya olarak indirir.
 * @param {string} apiId
 * @param {'json'|'txt'} format
 */
function exportResult(apiId, format) {
  const body = document.getElementById(`result-body-${apiId}`);
  if (!body || !body.textContent.trim()) {
    showToast("İndirilecek sonuç bulunamadı.", "warning");
    return;
  }
  ...
  const filename = `hapsetme_${apiId}_${Date.now()}.${ext}`;
  ...
  showToast(`Sonuç ${ext.toUpperCase()} olarak indirildi.`, "success");
}
...
const FAVS_KEY = "hapsetme_favorites";
...
function isFavorite(apiId) {
  return getFavorites().includes(apiId);
}

function toggleFavorite(apiId) {
  let favs = getFavorites();
  if (favs.includes(apiId)) {
    favs = favs.filter((f) => f !== apiId);
...
function updateFavBtn(apiId) {
  const btn = document.getElementById(`btn-fav-${apiId}`);
  if (!btn) return;
  const fav = isFavorite(apiId);
  btn.classList.toggle("active", fav);
  btn.title = fav ? "Favorilerden çıkar" : "Favorilere ekle";
  btn.innerHTML = `<i class="fa-${fav ? "solid" : "regular"} fa-star"></i>`;
}
...
function renderFavoritesSection() {
  ...
  grid.innerHTML = favs.map((apiId) => {
    // API_DEFINITIONS'dan ismi bul
    const api = (typeof API_DEFINITIONS !== "undefined")
      ? API_DEFINITIONS.find((a) => a.id === apiId)
      : null;
    if (!api) return "";
    return `
      <button class="fav-chip" onclick="scrollToCard('${api.id}')">
        <i class="${api.icon}"></i>
        <span>${api.name}</span>
        <i class="fa-solid fa-arrow-right fav-chip-arrow"></i>
      </button>
    `;
  }).join("");
}

function scrollToCard(apiId) {
  const card = document.getElementById(`card-${apiId}`);
  if (!card) return;
