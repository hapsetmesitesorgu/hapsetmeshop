...
async function queryApi(apiId) {
  ...
  try {
    ...
    // Not: Tarayıcı güvenliği nedeniyle farklı origin'e doğrudan istek
    //      CORS hatası verebilir. Proxy kullanılır.
    ...
    if (err.name === "AbortError") {
      message = { error: "İstek zaman aşımına uğradı.", details: "Sunucu yanıt vermedi." };
    } else {
      message = { error: "Sorgu başarısız.", details: err.message };
    }

    renderResult(apiId, message, "error", elapsed);
    showToast("Sorgu başarısız. Detaylar için sonuca bakın.", "error");
  } finally {
